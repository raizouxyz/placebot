...
client = CapmonsterClient(api_key=capmonster_apikey)
...
if check_mode:
    charges_total = 0
    for wplace_token in accounts:
        response = session.get("https://backend.wplace.live/me", headers=get_headers(wplace_token), proxies=random.choice(proxies))
        ...
        if response.status_code == 200:
            ...
            purchasable_charges = int(response.json()["droplets"]/500)*30
            charges_total += charge + purchasable_charges
            print(f"[Information] {user_name}({user_id}) 配置可能:{charge}/{charge_max}(購入可能:{purchasable_charges}) Droplets:{response.json()['droplets']}")
        else:
            accounts.remove(wplace_token)
    print(f"合計配置可能数:{charges_total} アカウント数:{len(accounts)}")
    exit()
...
while True:
    for wplace_token in accounts:
        ...
        if response.status_code == 200:
                ...
                for y in range(rgb_img.size[1]):
                    ...
                    if (x == rgb_img.size[0] - 1 and y == rgb_img.size[1] - 1) or (charge < 1):
                        ...
                        if len(request_data["colors"]) > 0:
                            response = session.post(f"https://backend.wplace.live/s0/pixel/{chunk_x}/{chunk_y}", headers=get_headers(wplace_token), proxies=random.choice(proxies),  data=json.dumps(request_data))
                            print(response.status_code, response.text)
                            if response.status_code == 200:
                            ...
                            if droplets >= 500 and charge < (charge_max - 32):
                                response = session.post("https://backend.wplace.live/purchase", headers=get_headers(wplace_token), proxies=random.choice(proxies), data=json.dumps({"product":{"id":80,"amount":1}}))
                        ...
                        #response = session.get("https://backend.wplace.live/me", headers=get_headers(wplace_token))
                        #charge = int(response.json()["charges"]["count"])
                    if nextaccount:
                        break
                if nextaccount:
            ...
            print("アホ", response.status_code, response.text)